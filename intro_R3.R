...
getwd()
hony <- read.csv("~/Desktop/Penn State University /Spring 2021/PLSC 496/TAD_2021-master/R lessons/humansofnewyork.csv", stringsAsFactors=FALSE) # read in data, character vectors treated as character vectors (not factors)
...
TotalLikesEcMonth = hony$likes_count + hony$created_time
monthsLikes = substr(hony$created_time,6,7)# extracts months from date created variable
new1 = tapply(hony$shares_count, monthsLikes, sum) # extracts year from date created variable
new1