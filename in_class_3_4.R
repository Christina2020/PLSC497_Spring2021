...
setwd("~/Desktop/Penn State University/Spring 2021/PLSC 496")
...
# load data
news_data <- readRDS("~/Desktop/Penn State University/Spring 2021/PLSC 496/news_data.rds")
...
> # take a look at the most discriminant features (get some face validity)
  > posterior <- data.frame(feature = rownames(t(nb_model_sm$param)), 
                            ...
                            +                         post_SPORTS = t(nb_model_sm$param)[,2])
> head(arrange(posterior, -post_SPORTS))
feature   post_CRIME post_SPORTS
video    video 0.0033765960 0.005980758
nfl        nfl 0.0001055186 0.005547369
olymp    olymp 0.0001055186 0.005460692
player  player 0.0007386304 0.005287336
game      game 0.0002110372 0.005113981
fan        fan 0.0001055186 0.003900494
> head(arrange(posterior, -post_CRIME))
feature  post_CRIME  post_SPORTS
man       man 0.010551862 0.0014735200
polic   polic 0.010129788 0.0005200659
kill     kill 0.009180120 0.0003467106
shoot   shoot 0.007913897 0.0006934212
alleg   alleg 0.005064894 0.0006934212
say       say 0.004959375 0.0028603623
'''''
#.2
'''
Baseline Accuracy:  0.5916151 
...
#When we use .2 is that a .8 it changes the trajectory of our accuracy line and we move further away from it.
...
#The Prior argument does: prior distribution on texts; one of "uniform", "docfreq", or "termfreq". 
#refer to the prior probabilities assigned to the training classes, and the choice of prior distribution 
#affects the calculation of the fitted probabilities. The default is uniform priors, which sets the unconditional 
#probability of observing the one class to be the same as observing any other class.
#The default value for this argument in quanteda character vector
